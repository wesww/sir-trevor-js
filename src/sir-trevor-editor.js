...
var stlog = require('./helpers/logger');
...
Object.assign(Editor.prototype, require('./function-bind'), require('./events'), {
  ...
  createBlock: function(type, data, render_at) {
    ...
    if(this._blockLimitReached()) {
      stlog("Cannot add any more blocks. Limit reached.");
      return false;
    }

    if (!this._isBlockTypeAvailable(type)) {
      stlog("Block type not available " + type);
      return false;
    }

    // Can we have another one of these blocks?
    if (!this._canAddBlockType(type)) {
      stlog("Block Limit reached for type " + type);
      return false;
    }
    ...
    stlog("Block created of type " + type);
  ...
  performValidations : function(block, should_validate) {
    ...
    if (!config.skipValidation && should_validate) {
      if(!block.valid()){
        ...
        stlog("Block " + block.blockID + " failed validation");
        ++errors;
      }
    }
  ...
  saveBlockStateToStore: function(block) {
    ...
    if(store && !_.isEmpty(store.data)) {
      stlog("Adding data for block " + block.blockID + " to block store");
  ...
  onFormSubmit: function(should_validate) {
    ...
    stlog("Handling form submission for Editor " + this.ID);
  ...
  validateBlockTypesExist: function(should_validate) {
    ...
    var blockTypeIterator = function(type, index) {
      ...
      if (this._getBlockTypeCount(type) === 0) {
        stlog("Failed validation on required block type " + type);
        ...
        stlog("A required block type " + type + " is empty");
      }
    };
